...
// Vendored from Deno v2.0.0 cli/napi/node_api.rs and adapted for howth.
...
#[napi_sym]
fn napi_remove_async_cleanup_hook(remove_handle: napi_async_cleanup_hook_handle) -> napi_status {
    if remove_handle.is_null() {
        return napi_invalid_arg;
    }

    let handle = unsafe { Box::<AsyncCleanupHandle>::from_raw(remove_handle as _) };
...
fn napi_fatal_error(
    ...
    let location = if location.is_null() {
        ...
        unsafe {
            Some(if location_len == NAPI_AUTO_LENGTH {
                ...
                let slice =
                    std::slice::from_raw_parts(location as *const u8, location_len as usize);
    ...
    let message = if message_len == NAPI_AUTO_LENGTH {
        ...
        let slice =
            unsafe { std::slice::from_raw_parts(message as *const u8, message_len as usize) };
        std::str::from_utf8(slice).unwrap()
    };
...
#[napi_sym]
fn napi_close_callback_scope(env: *mut Env, scope: napi_callback_scope) -> napi_status {
    let env = check_env!(env);
...
#[napi_sym]
fn napi_async_destroy(env: *mut Env, async_context: napi_async_context) -> napi_status {
    let env = check_env!(env);
...
fn napi_make_callback<'s>(
    ...
    let Some(func) = func.and_then(|v| v8::Local::<v8::Function>::try_from(v).ok()) else {
...
fn napi_create_buffer<'s>(
    ...
    let buffer_constructor = v8::Local::new(&mut env.scope(), &env.buffer_constructor);
    let Some(buffer) = buffer_constructor.new_instance(&mut env.scope(), &[ab.into()]) else {
        return napi_generic_failure;
    };
...
#[napi_sym]
fn napi_is_buffer(env: *mut Env, value: napi_value, result: *mut bool) -> napi_status {
    let env = check_env!(env);
    check_arg!(env, value);
    check_arg!(env, result);
...
fn napi_get_buffer_info(
    ...
    // in Node.js.
    let Some(ta) = value.and_then(|v| v8::Local::<v8::TypedArray>::try_from(v).ok()) else {
...
fn napi_create_async_work(
    ...
    let Some(resource_name) = async_resource_name.and_then(|v| v.to_string(&mut env.scope()))
    else {
...
#[napi_sym]
fn napi_get_uv_event_loop(env_ptr: *mut Env, uv_loop: *mut *mut ()) -> napi_status {
...
impl TsFn {
    ...
    pub fn release(tsfn: *mut TsFn, mode: napi_threadsafe_function_release_mode) -> napi_status {
    ...
    pub fn call(&self, data: *mut c_void, mode: napi_threadsafe_function_call_mode) -> napi_status {
        if self.is_closing.load(Ordering::SeqCst) {
            return napi_closing;
        }
        ...
        self.sender.spawn(move |scope: &mut v8::HandleScope| {
            ...
            if is_closed.load(Ordering::Relaxed) {
                ...
                unsafe {
                    (tsfn.call_js_cb)(tsfn.env as _, func.into(), tsfn.context, data as _);
                }
            }
        });
...
#[napi_sym]
fn napi_acquire_threadsafe_function(tsfn: napi_threadsafe_function) -> napi_status {
    assert!(!tsfn.is_null());
...
#[napi_sym]
fn napi_unref_threadsafe_function(_env: &mut Env, func: napi_threadsafe_function) -> napi_status {
    assert!(!func.is_null());
    let tsfn = unsafe { &*(func as *mut TsFn) };
    tsfn.unref()
}
