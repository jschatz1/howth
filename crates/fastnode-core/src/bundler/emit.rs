...
pub fn emit_bundle_with_entry(
    ...
    let used_exports = if options.treeshake && entry_id.is_some() {
        Some(UsedExports::analyze(graph, entry_id.unwrap()))
    } else {
        None
    };
    ...
    match options.format {
        BundleFormat::Esm => emit_esm(graph, order, options, used_exports.as_ref(), entry_id, &mut output)?,
        BundleFormat::Cjs => emit_cjs(graph, order, options, used_exports.as_ref(), entry_id, &mut output)?,
        BundleFormat::Iife => emit_iife(graph, order, options, used_exports.as_ref(), entry_id, &mut output)?,
    }
...
fn emit_module(
    ...
    let used_set: Option<HashSet<String>> = used_exports.and_then(|u| {
        u.get_used(id).cloned()
    });
...
fn transform_module(
    ...
    for line in transpiled.lines() {
        let (transformed, export_stmts) = transform_line_with_exports(line, module_path, graph, used_exports)?;
...
fn is_asset_import(spec: &str) -> bool {
    let asset_exts = [".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".ico", ".avif",
                      ".woff", ".woff2", ".ttf", ".otf", ".eot",
                      ".json", ".txt", ".wasm"];
    asset_exts.iter().any(|ext| spec.ends_with(ext))
}
...
fn rewrite_export_with_pending(line: &str, used_exports: Option<&HashSet<String>>) -> (String, Vec<String>) {
    let trimmed = line.trim();
        ...
        || trimmed.starts_with("export var ")
    {
        ...
        if parts.len() >= 2 {
            ...
            if is_used(name) {
                return (decl.to_string(), vec![format!("exports.{} = {};", name, name)]);
            } else {
        ...
        if let Some(end) = trimmed.find('}') {
            ...
            for name in export_names {
                if name.contains(" as ") {
                    ...
                    if parts.len() == 2 {
                        export_stmts.push(format!("exports.{} = {};", parts[1].trim(), parts[0].trim()));
                    }
...
mod tests {
    ...
    fn test_rewrite_import_named() {
        let graph = empty_graph();
        assert_eq!(
            rewrite_import("import { foo, bar } from './utils';", "/test/file.ts", &graph),
    ...
    fn test_tree_shaking_filters_unused() {
        ...
        let (_, exports) = rewrite_export_with_pending("export function unusedFn() {}", Some(&used));
        assert!(exports.is_empty());
    }
